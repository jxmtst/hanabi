...
メニューバーの 🎆 から弾幕の停止/再開・**表示方式の切替（スクロール/花火）**・
**表示画面（モニタ）の切替**・**アイコンの切替（なし/小/中/大/特大）**・終了ができる。
（`RENDERER` / `DISPLAY_INDEX` / `AVATAR_SCALE` は起動時の既定値。実行中はメニューから切り替えられる）
...
| `AVATAR_SCALE` | アイコンの大きさ（文字高に対する倍率。既定 1.6） |
